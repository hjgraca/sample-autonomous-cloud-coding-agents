...
- **Granularity boundary.** The orchestrator treats the whole session as one `await-agent-completion` step, so platform invariants stay agent-external ([ADR-014](../decisions/ADR-014-workflow-driven-tasks.md)). Recovery relies on task records and idempotent external effects rather than an SDK transcript.
