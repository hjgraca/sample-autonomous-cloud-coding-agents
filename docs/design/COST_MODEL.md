...
- **Per-task:** Token usage and compute duration are captured in task metadata (`agent.cost_usd`, `agent.turns` - see [OBSERVABILITY.md](./OBSERVABILITY.md)). `agent.cost_usd` is the Strands harness's **client-side estimate**, calculated from Bedrock token metrics and `agent/src/model_pricing.py`; it is not authoritative billing. Budgeted tasks reject models absent from that table before invocation. Use AWS Cost Explorer / CUR 2.0 for the real bill (see [COST_ATTRIBUTION.md](../guides/COST_ATTRIBUTION.md)).
